...
public class DictionaryDaoImpl implements DictionaryDao {
    ...
    private static final String GET_STREET = "SELECT street_code, street_name " +
            "FROM jc_street WHERE upper(street_name) LIKE upper(?)";// ? - это параметр, нумеруется с 1
    private static final String GET_PASSPORT = "SELECT * " +
            "FROM jc_passport_office WHERE p_office_area_id = ?";// ? - это параметр, нумеруется с 1
    private static final String GET_REGISTER = "SELECT * " +
            "FROM jc_register_office WHERE r_office_area_id = ?";// ? - это параметр, нумеруется с 1

    private static final String GET_AREA = "SELECT * " +
            "FROM jc_country_struct WHERE area_id like ? and area_id<>?";// ? - это параметр, нумеруется с 1
    ...
    public List<Street> findStreets(String pattern) throws DaoException {
        ...
        //Class.forName("org.postgresql.Driver"); //в jdbc 4 можно не загружать класс по имени

        try (Connection con = getConnection();
             PreparedStatement stmt = con.prepareStatement(GET_STREET))
            /*создаем запрос, в который передаем параметр*/ {
            //если коннект создастся, то зайдем в блок трай,
            // а в конце блока трай коннект автомат закроется

            stmt.setString(1, "%" + pattern + "%");//ставим номер параемтра 1, и само значение параметра
